...
export function SchedulePage({ locale }: { locale: Locale }) {
  ...
  return <main>
    ...
    <ImageTextSection locale={locale} copy={page.introduction} photo="practice-yoga">
      <a href="#booking" className="inline-flex min-h-11 items-center text-sm font-semibold text-forest underline underline-offset-4">{shared.bookingStatus}</a>
    </ImageTextSection>
    <section id="booking" className="mx-auto max-w-7xl scroll-mt-8 px-5 sm:px-8 lg:px-12">
      <AcuityScheduler title={page.booking.title} placeholder={page.booking.body} ctaLabel={shared.instagram} ctaHref={social.instagram.url} />
    </section>
    <EditorialSection title={shared.locationShortcuts}><LocationLinks locale={locale} compact /></EditorialSection>
    <div className="bg-soft-white"><EditorialSection title={page.stepsTitle}>
      ...
      <ol className="mt-10 grid gap-10 md:grid-cols-3">{page.steps.map((step, index) => <li key={step.title} className="border-t border-forest/20 pt-6">
        ...
        <h3 className="mt-4 font-display text-3xl text-forest">{step.title}</h3><p className="mt-4 leading-7 text-charcoal/75">{step.body}</p>
      </li>)}</ol>
