...
export function ImageTextSection({ locale, copy, photo, children }: { locale: Locale; copy: EditorialCopy; photo: keyof typeof imageAlt; children?: ReactNode }) {
  return <section className="mx-auto grid max-w-7xl items-center gap-10 px-5 py-16 sm:px-8 sm:py-24 md:grid-cols-2 md:gap-16 lg:px-12">
    <figure>
      <div className="relative aspect-[4/5] overflow-hidden rounded-2xl">
        <Image src={`/photos/${photo}.webp`} alt={imageAlt[photo][locale]} fill quality={85} sizes="(min-width: 1280px) 560px, (min-width: 768px) 45vw, 100vw" className="object-cover object-center" />
      </div>
      <figcaption className="mt-4 max-w-lg text-xs leading-5 text-charcoal/65">{secondaryContent[locale].shared.photoCaption}</figcaption>
    </figure>
    <div>
...
export function CTASection({ locale }: { locale: Locale }) {
  const copy = secondaryContent[locale].shared;
  return <section className="bg-soft-white px-5 py-16 sm:px-8 sm:py-20 lg:px-12">
    <div className="mx-auto max-w-7xl items-center justify-between gap-10 md:flex">
      <div><h2 className="max-w-2xl font-display text-4xl leading-tight text-forest sm:text-5xl">{copy.cta.title}</h2><p className="mt-5 max-w-xl leading-7 text-charcoal/75">{copy.cta.body}</p></div>
      <div className="mt-8 shrink-0 md:mt-0"><ScheduleLink locale={locale} /></div>
    </div>
  </section>;
}
